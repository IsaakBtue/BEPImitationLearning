...
def hand_proximity_strict(
    ...
    Mirrors upstream _reward_success exactly:
        return (success_flag + 1.0) * (dist < strict_th)
    Returns 1.0 while hand is close; doubles to 2.0 after the ball has been
    stopped (_sb_flag set), rewarding the robot for keeping its hand on the
    ball post-save. Weight: 5.0 (same as upstream).
    """
    robot: Entity = env.scene[asset_cfg.name]
    ball: Entity = env.scene[ball_name]
    hand_pos_w = robot.data.body_link_pos_w[:, asset_cfg.body_ids, :]
...
def torques_normalized_l2(
    ...
    The kp vector is built once from T1_KP_MAP and cached on the env.
    """
    robot: Entity = env.scene[asset_cfg.name]
...
def deviation_waist_joint(
    ...
    """Penalize waist joint squared deviation from default position at all times.

    Mirrors upstream _reward_deviation_waist_pitch_joint exactly:
        return sum(square(dof_pos - default)[:, waist_pitch_idx])
    G1 had a dedicated waist_pitch joint; T1 has a single Waist joint.
    Uses squared error (not abs) to match the original formula.
