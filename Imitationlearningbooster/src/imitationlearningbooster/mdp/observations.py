...
def joint_pos_amp(env: ManagerBasedRlEnv) -> torch.Tensor:
    """Raw joint positions for AMP discriminator — single frame, no noise, no history.

    Returns shape (num_envs, 23). Registered under the 'amp' ObservationGroup.
    Used by the AMP module to discriminate between expert motions and policy behavior.
    """
    robot: Entity = env.scene["robot"]
