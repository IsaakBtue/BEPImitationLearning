...
def eereach(
    ...
    dist = torch.norm(hand_pos_w - ball_pos_w[:, None, :], dim=-1)  # (N, 2)
...
def stopball(
    ...
    """Reward when ball in front of goal line has slowed or reversed (stopped/deflected)."""
    ball: Entity = env.scene[ball_name]
    ball_x_local = ball.data.root_link_pos_w[:, 0] - env.scene.env_origins[:, 0]
    ball_x_vel = ball.data.root_link_lin_vel_w[:, 0]
    in_front = ball_x_local > 0.0
    deflected = ball_x_vel > -0.5  # slowed or reversed
...
def stayonline(
    ...
    """Penalty for robot drifting away from the goal line (x = 0 in local frame)."""
...
def noretreat(env: ManagerBasedRlEnv) -> torch.Tensor:
    """Penalty for retreating (negative x velocity in world frame toward own goal)."""
    robot: Entity = env.scene["robot"]
...
def feetorientation(
    ...
    feet_quat_w = robot.data.body_link_quat_w[:, asset_cfg.body_ids, :]  # (N, 2, 4)
    gravity_w_exp = gravity_w[:, None, :].expand(-1, 2, -1)
    gravity_foot = quat_apply(quat_inv(feet_quat_w), gravity_w_exp)  # (N, 2, 3)
    err = torch.sum(gravity_foot[..., :2] ** 2, dim=-1).sum(dim=-1)  # (N,)
