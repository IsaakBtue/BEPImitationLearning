...
## 2026-07-05 — blue-ball hard gate had a hole: footreach/foot_proximity's ball_close override bypassed it unconditionally

**What changed:** `footreach` and `foot_proximity`'s `ball_close = ball_x_local < 0.5` live-ball switch is now gated `& (~env._blue_wide | env._blue_landed)` on wide crossings. `_get_reach_target_y` caches the `wide` mask as `env._blue_wide` (previously computed locally and discarded) so the two reward functions can read it without recomputing crossing geometry. Narrow crossings are unaffected — `env._blue_wide` is false there, so the switch behaves exactly as before. No time-based fallback was added.

**Why it was wrong:** the 2026-07-04 hard gate (previous entry) removed `_get_reach_target_y`'s own time-based fallback, but `footreach`/`foot_proximity`'s separate `ball_close` switch was left unconditional — the docstring even called this out as the intended "backstop against stalling." In practice this backstop swallowed the entire point of the hard gate: once the ball closed to within 0.5 m of the goal line, the reward target snapped straight to the live ball position regardless of whether `env._blue_landed` was ever true, so a policy that ran straight for the true crossing point without ever visiting the blue midpoint paid no penalty for skipping it. Compounding this, `footreach`'s per-step dive reward (weight 10→25, up to 10x via `vel_sigma`, applied every step of the approach) is roughly two orders of magnitude larger over an episode than `blue_ball_landed`'s one-shot bonus (same weight range, fires once) — so even during the brief window before `ball_close` activated, skipping the midpoint and heading straight for the true target was the dominant strategy. User caught this by direct observation in `sgk_play`: the robot always ran straight to the green/full target and only ever made foot contact there, never landing at the blue midpoint first — confirming the `~21%` `blue_landed_genuine` rate seen in run `2026-07-04_13-59-57` was mostly incidental mid-stride footfalls landing within the 0.3 m radius by geometric coincidence (the midpoint sits directly on the path to the true target), not deliberate stepping-stone behavior.

**Rejected alternative — add a timing safety valve:** considered gating `ball_close` on `_blue_landed` OR an elapsed-time escape hatch (advance the target without reward past ~1.5x flight time) to guard against episodes stalling on an unreachable midpoint. Rejected as inconsistent — `_get_reach_target_y` itself already accepted the "stays on the midpoint for the whole episode if never landed" behavior with no fallback when the 2026-07-04 hard gate shipped; reintroducing a fallback here just for this call site would contradict that already-accepted design rather than closing the loophole cleanly.

**Correct value:** `ball_close = (ball_x_local < 0.5) & (~env._blue_wide | env._blue_landed)` in both `footreach` and `foot_proximity`.

**Evidence:** two new regression tests, `test_footreach_ball_close_does_not_bypass_landing_gate_on_wide_crossing` and `test_foot_proximity_ball_close_does_not_bypass_landing_gate_on_wide_crossing` (`tests/simple_goalkeeper/test_footreach_two_stage_wiring.py`), confirmed failing against the pre-fix code (foot at midpoint scored 0.31 vs. 0.72 for foot at the full point, i.e. the target had already snapped live) and passing after the fix. Companion tests confirm `ball_close` still switches to the live target once `_blue_landed` is genuinely true, and that narrow crossings are unaffected. Not yet validated against a training run.

---

## 2026-07-05 — stopball/softstop never referenced the blue-ball landing gate at all: the primary save reward could skip the waypoint entirely

**What changed:** `stopball` and `softstop` (`mdp/rewards.py`) now call `_get_reach_target_y(env, ball_name)` at the top of each (ensuring `env._blue_wide`/`env._blue_landed` are fresh this step, same pattern `blue_ball_landed` already uses) and AND `landing_ok = ~env._blue_wide | env._blue_landed` into their `fired` condition. Since `env._sb_flag |= fired` and `env._softstop_flag |= fired`, the underlying flags themselves — not just each function's own returned reward — only latch once landing has genuinely happened on wide crossings. Narrow crossings (`~env._blue_wide`) are completely unaffected. Gating the flags (rather than only each function's returned value) was a deliberate choice: `_softstop_flag`/`_softstop_correct_foot` also gate `_ball_is_behind` and every same-foot bonus (`single_foot_save`, `cleanstop`, `inner_face_orientation_save`, `foot_inner_face_continuous`), so this makes the entire save-and-recover pipeline consistently wait on the waypoint rather than just withholding two reward numbers while everything downstream still behaved as if the save had happened. `_ball_is_behind`'s independent `ball_x_local < 0` condition means nothing can stall forever — it still fires once the ball physically crosses the goal line even if no wide-crossing save landed there first.

**Why it was wrong:** the previous two fixes (2026-07-04, 2026-07-05 above) closed every path by which the two-stage *target* could be bypassed, but neither of them touched `stopball`/`softstop` themselves — those two rewards fire purely off ball physics (`delta_vx`, `ball_x_vel`) with zero reference to `env._blue_wide`/`env._blue_landed`. Confirmed by user observation in `sgk_play` (checkpoint `model_4750`, run `2026-07-04_20-31-10`, played without `--rsi` so no RSI-seeded free credit was in play): on wide crossings the robot made one continuous "big reach" straight to the true crossing point with no visible two-stage stepping. Root cause investigation (this file's own `Episode_Metrics/blue_landed_genuine` for that run: ~23-26% of ALL episodes, narrow included) confirmed genuine landing is a clear minority even before restricting to wide-only, while `Episode_Reward/softstop`/`stopball` (~262.5/~37.5 max curriculum weight) are roughly an order of magnitude larger than `blue_ball_landed` (max weight 25, one-shot). The optimal policy on a wide crossing was therefore to skip the waypoint and beeline for the true crossing point: the small shaping/bonus rewards for missing the midpoint were dominated by the certainty of collecting the full save reward anyway.

**Correct value:** `landing_ok = ~env._blue_wide | env._blue_landed`, ANDed into both `stopball`'s and `softstop`'s `fired` computation, each preceded by a fresh `_get_reach_target_y(env, ball_name)` call.

**Risk, accepted not resolved:** this run's `blue_landed_genuine` (~24% of all episodes) suggests the wide-crossing-only genuine-landing rate is well under 50%. Hard-gating the two largest rewards in the reward table behind that rate risks sparsifying training signal sharply on exactly the harder subset of episodes (wide crossings) — the same category of risk the 2026-07-04 hard gate itself went through an iteration cycle over. User's explicit choice to ship the gate now and watch the next run's `stopball`/`softstop`/`blue_landed_genuine` episode rates and any divergence, rather than soften it preemptively.

**Evidence:** six new tests in `tests/simple_goalkeeper/test_stopball_softstop_landing_gate.py` (wide+never-landed blocks both rewards, wide+landed fires both, narrow is unaffected for both). 84/84 tests pass. Not yet validated against a training run.

---

## 2026-07-07 — `reset_ball_rolling` computed `env._rsi_cross_y` twice, the second (wrong) write silently overwrote the first (correct) one

**What changed:** removed the second, duplicate `env._rsi_cross_y[env_ids] = ...` block near the end of `reset_ball_rolling` (`mdp/events.py`). The correct computation, `y_start + dy * (x_start / (x_start + 0.3))`, already exists earlier in the same function (right after `dx`/`dy` are computed) and is now the only one left.

**Why it was wrong:** the function set `env._rsi_cross_y` twice. The first assignment used the correct crossing fraction — the ball travels in a straight line from `(x_start, y_start)` to `(-0.3, y_end)`; parametrizing by `f ∈ [0,1]`, `X(f) = x_start - f*(x_start+0.3)`, and the goal line is at `X=0` (not at the aim point `X=-0.3`), so `f = x_start/(x_start+0.3)`. The second, later assignment recomputed the same variable using `f = (x_start+0.3)/horiz_dist` where `horiz_dist = sqrt((x_start+0.3)² + (y_end-y_start)²)` — that's `cos(trajectory angle)`, not a crossing fraction at all, and it silently clobbered the correct value directly above it. Numerically the two disagree on every diagonal shot: e.g. `x_start=2.0, dy=0.5` gives correct `f=0.870` vs. buggy `f=0.977`, i.e. the buggy version places `cross_y` closer to `y_end` than it should be. Found while investigating a user-reported "near balls also get the blue/green two-stage split" symptom in `interceptV2DualDis` (a near-verbatim port of this file) — turned out the same duplicate-write bug exists here too, most likely the origin the port copied from. Direct empirical testing (95 sampled resets, forcing max difficulty) showed the `wide` flag itself is 100% consistent with `|env._rsi_cross_y| > wide_threshold` before and after this fix — the bug is mathematically bounded (`f` is always in `(0,1]` for both formulas, so `cross_y` can never leave `[min(y_start,y_end), max(y_start,y_end)]`), so it was never capable of flipping a legitimately-narrow crossing to "wide". It's a real accuracy bug in the target/waypoint geometry for every diagonal shot regardless, not a classification bug.

**Correct value:** `env._rsi_cross_y[env_ids.long()] = y_start + dy * (x_start / (x_start + 0.3))` — the single, first assignment in the function; the later duplicate block is deleted entirely (its local `horiz_dist` variable was unused elsewhere).

**Evidence:** 84/84 existing tests still pass unchanged (no test exercised the exact numeric value of `_rsi_cross_y`, only the downstream `wide` flag, which is unaffected by construction — see above). Re-ran the same 95-episode empirical wide-flag check after the fix: still 0 mismatches. Companion fix in `interceptV2DualDis/src/simple_goalkeeper/mdp/events.py` (which only ever had the buggy formula, no duplicate) applied the same day.
