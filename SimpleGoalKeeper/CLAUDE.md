...
| Ball spawn `y_end_range` (goal target) for rolling ball | ±0.5 m (previous) | **±1.5 m** | **FIX 2026-06-20:** ±0.5 m combined with the 0.3 m footreach dead-zone meant ~60-100% of balls aimed within the dead-zone at easy/medium difficulty → robot got phase1 footreach=1.0 for free by standing still at center. Widened to ±1.5 m so the robot must laterally step to intercept wide balls. Curriculum lerps from easy ±0.05 m to hard ±1.5 m so early training stays centered. `goalkeeper_env_cfg.py:reset_ball`. |
...
| Ball speed range | G1: varies by task, typically 3–6 m/s | **Easy: (0.8, 1.5), Hard: (0.8, 2.0) m/s** | **FIX 2026-06-24:** Previous easy range was `_EASY_SPEED_R = (1.5, 2.5)` with hard `speed_range = (0.5, 1.5)`. This INVERTED difficulty — easy balls (d=0) were 1.5–2.5 m/s, hard balls (d=1) were 0.5–1.5 m/s. Slower balls cannot overcome stopball COR physics. Fix: `_EASY_SPEED_R = (0.8, 1.5)`, `speed_range = (0.8, 2.0)`. Now speed monotonically increases with difficulty. Min 0.8 m/s ensures stopball at 0.6 threshold is always reachable. `events.py:_EASY_SPEED_R`, `goalkeeper_env_cfg.py:speed_range`. |
