...
| `footreach` phase2 target | Live ball position | **Frozen crossing point** `(goal_x, crossing_y, floor+0.12)` | **FIX 2026-06-20:** Using live ball position gave ~0.82 reach_rew when ball rolled past a standing foot at center (foot-to-ball dist ≈ 0.04 m at contact moment) — free reward without any interception. Switched to frozen crossing point (same as `foot_proximity`), mirroring ILB eereach's `end_target`. Now foot must pre-position at the exact arrival location to get high reward. `rewards.py:footreach`. |
