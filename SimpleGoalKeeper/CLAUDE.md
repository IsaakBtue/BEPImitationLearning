...
| RSI mechanism (`reset_from_motion_data`) | `continue_keep` branch (80%): single batch-level coin flip; copy `dof_pos` from `torch.randint(0, num_envs, ...)` (any live env, no tier matching, no exclusion of current reset batch, no maturity check), no clamp. Else branch (20%, `randomize_initial_joint_pos=True` in G1's active g1_29_config): `standpos * U(0.5, 1.5) + U(-0.1, 0.1)`, clipped to HARD `dof_pos_limits`. `dof_vel` always zeroed both branches; root untouched (G1 also randomizes root velocity ±0.3 unconditionally in `_reset_root_states`, separate from this). | **Literal port**, matching every point above: one `torch.rand(1)` draw per `reset()` call, donor from any of `env.num_envs` with no restriction/guard and no clamp; else branch scales/offsets `default_joint_pos` and clips to `robot.data.joint_pos_limits` (hard, not soft); `joint_vel` always zeroed; root left to `reset_base` (root velocity still zero there — known, separate, out-of-scope divergence). | **2026-07-01:** Initially implemented as a side/tier-scoped variant, then explicitly reverted to a literal, unscoped port per user request. A follow-up independent fidelity audit (dispatched per user request to check "is this actually similar to G1") caught three further divergences in that first literal-port attempt, all now fixed: (1) the clamp was applied to the donor-copy branch, where G1 has none, and was missing from the else branch, where G1's active config actually clips; (2) the else branch was a flat `default_joint_pos` copy with zero randomization, omitting G1's active `randomize_initial_joint_pos=True` scale/offset entirely; (3) `reset_from_motion_data` hardcoded `rsi_fraction=0.5`, silently running a 50/50 split instead of G1's 80/20 the whole time since 2026-06-20. The old side/tier NPZ-pool system (`self.pools`, `_write_rsi_state`, `_STEM_TO_POOL`) still exists and loads at startup but is no longer used by `reset()` — kept only for the `sgk_play_rsi` diagnostic script. See `docs/superpowers/plans/2026-07-01-live-env-rsi.md` for the full audit trail. **UPDATE 2026-07-03 — three-way split (user request, deliberate divergence from the literal port):** `reset()` now draws once per call and picks: 30% ball-conditioned NPZ tier RSI (`_tier_npz_reset`: routes by the predicted goal-line crossing Y that `reset_ball_rolling` now stores in `env._rsi_cross_y`; |cy|<0.20 → standing HOME, 0.20–0.40 → double pool (SafeMedium), 0.40–0.60 → triple pool (SafeFar), ≥0.60 → wide pool (DoubleStep+TripleStep); writes FULL RSI state via `_write_rsi_state`), 50% G1 continue_keep live donor (unchanged literal port), 20% G1 randomized standing (unchanged). Motivation: the policy was not converging to double-stepping — the literal port removed the only mechanism that seeded double/triple-step poses on wide balls. The 80/20 RSI-vs-standing share is preserved (0.3+0.5 = G1's 0.8). Play mode still pops the event. **UPDATE 2026-07-03 (later same day) — fractions raised 30/50/20 → 50/30/20:** `tier_rsi_fraction` 0.3→0.5, `live_rsi_fraction` 0.5→0.3 in `MotionResetManager.reset()`'s and `reset_from_motion_data()`'s defaults, and in `goalkeeper_env_cfg.py`'s `reset_from_motion_data` params dict. Motivation: the tier-agnostic 50% `continue_keep` donor branch copies `dof_pos` from an arbitrary live env with no regard for that donor's own ball width, so only 30% of wide-crossing episodes were actually being seeded with a deliberate double/triple-step pose — the other 70% got an unrelated donor pose or plain standing, diluting exposure to the harder behavior during exactly the exploration phase that needed it most. Raising the tier share to 50% directly increases that exposure with zero new reward-hacking surface (it only changes which pre-scripted pose an episode starts from). The 80/20 RSI-vs-standing share is unchanged. Also fixed: `scripts/play.py`'s `--rsi` flag re-registers `reset_from_motion_data` with no params dict, so it was silently relying on (and now correctly picks up) these function defaults rather than the training config's params. `tests/simple_goalkeeper/test_live_rsi.py` updated (`test_reset_partition_boundaries_at_default_fractions`, `test_reset_three_way_split_statistics`, `test_reset_from_motion_data_passes_the_three_way_split_to_reset`, `test_reset_single_coin_flip_covers_the_whole_batch_at_once`). `events.py:MotionResetManager.reset/_tier_npz_reset`, `reset_ball_rolling`, `goalkeeper_env_cfg.py`. |
| `footreach`/`foot_proximity` reach target on wide crossings | N/A — G1 has no staged/waypoint target mechanism (checked `legged_gym/legged_gym/envs/base/legged_robot.py` and `g1/g1_29_config.py`: no stepping, air-time, or intermediate-target reward of any kind exists). G1's footwork emerges purely from wide command-range targets + step motions in the AMP prior. | **Two-stage target (2026-07-03, user-directed design):** for crossings where `|crossing_y - start_y| > 0.6` (same threshold `_tier_npz_reset` uses for its DoubleStep/TripleStep RSI pool), `footreach`/`foot_proximity` target the MIDPOINT between the robot's stance and the true crossing point for the first half of the ball's flight time (`env._ball_t_flight`, newly cached in `reset_ball_rolling`), then switch to the full crossing point for the second half. Narrow crossings (`<= 0.6`) always target the full point, unchanged. New helper `mdp.rewards._get_reach_target_y`. | **Deliberate divergence, justified by direct investigation:** widening `y_end_range` to ±1.1 alone (2026-07-03, earlier same day) did not produce double-stepping — the policy converged on one continuous maximal-speed leap straight to the final target instead. Root cause: the AMP discriminator (`beyondAMP/.../motion_dataset.py`) only ever builds `(s_t, s_{t+1})` 2-frame transitions, so it has no mechanism to judge global trajectory shape or step count — a smooth fast leap satisfies it as well as a genuine multi-step reference clip — and `footreach`'s `vel_sigma` (up to 10×) rewards raw speed toward a single fixed target, not gait, so nothing in the existing reward pushed toward a paced multi-step approach. Reference-clip measurement confirmed the leap isn't a leg-reach trick either: max lateral foot-extension relative to the trunk tops out ~0.28–0.32 m in every step/double/triple `.npz` clip, so genuine multi-step travel is required past that distance. Scheduling an intermediate waypoint by flight time reuses the existing, already-tuned `footreach`/`foot_proximity` reward instead of adding a new one-shot term (rejected alternative: a contact-detection-based one-shot "intermediate step" bonus — more reward-hacking surface, no reuse of proven reward machinery, and this project has previously and explicitly worried that "a crafted step reward invites shuffle hacking", see the `y_end_range` row above). Self-correcting by construction: once the schedule flips to the full point, the sigmoid reach term and the up-to-10× `vel_sigma` multiplier both collapse for a foot still sitting at the (now stale) midpoint, so lingering at the midpoint is not rewarded — no separate exploit guard was added. `is_left_ball` foot-side selection in both reward functions intentionally still uses the TRUE crossing_y, not the staged target, so the assigned foot never flips mid-episode. Root XY is always pinned to `env.scene.env_origins` at reset (`reset_base`; `_write_rsi_state` only ever overwrites Z), so "robot start Y" needed no new per-episode cache. Play-mode visualization added in `scripts/play.py` (`_patch_viewer_intercept_vis`): a blue sphere at the midpoint during phase 1, replaced by the existing green sphere at the full point once the schedule flips — lets a human confirm the timing visually via `sgk_play`. Covered by `tests/simple_goalkeeper/test_reach_target_two_stage.py` (11 tests) and `tests/simple_goalkeeper/test_footreach_two_stage_wiring.py` (4 tests). `mdp/rewards.py:_get_reach_target_y,footreach,foot_proximity`, `mdp/events.py:reset_ball_rolling` (t_flight caching), `scripts/play.py:_patch_viewer_intercept_vis`. |
| `footreach`/`foot_proximity` reach target: landing gate on the two-stage schedule | N/A — G1 has no staged/waypoint target mechanism at all (see the parent two-stage-target row above); a landing-gated version of a mechanism that doesn't exist upstream has no G1 equivalent either. | **Landing gate (2026-07-04, hardened same day):** the blue→green switch on wide crossings now ONLY fires on a genuine foot landing — the assigned foot must be airborne at some point then land in ground contact within 0.3 m of the blue midpoint target (`feet_contact` sensor). The original `elapsed >= t_flight/2` time-based fallback was REMOVED (not just supplemented) after research showed the soft-gated version's ~35-40% apparent landing rate (correctly converted via the `Episode_Reward` formula below) was mostly explained by RSI-seeded "free" landings, not learned behavior — confirmed by ~0% genuine landings visible in `sgk_play`. `blue_ball_landed` (one-shot bonus, now curriculum-ramped 10→25 like `footreach` rather than flat +10) pays out the first time the landing latch fires. New diagnostic metrics `blue_landed_genuine`/`blue_landed_rsi_assisted` (`mdp/metrics.py`, `Episode_Metrics/*`, no weight/dt scaling) classify each landing using a new latch, `env._blue_airborne_at_reset` (true if the assigned foot's first airborne transition happens within 2 steps of reset). `footreach`/`foot_proximity`'s separate `ball_close < 0.5 m` live-ball override is unaffected and remains the true backstop against stalling. | **Deliberate divergence:** the 2026-07-03 two-stage schedule assumed the sigmoid reach term and vel_sigma multiplier would "self-correct" against lingering at the midpoint without a separate landing check — in practice the policy glided/leapt through the midpoint without ever placing a foot near it, since nothing checked it actually arrived before the time-based switch advanced anyway. Gating the switch on a genuine airborne-then-contact event forces the intermediate step to actually happen. See `docs/superpowers/specs/2026-07-04-blue-ball-landing-gate-design.md`. Covered by `tests/simple_goalkeeper/test_blue_ball_landing_gate.py` (7 tests: no-landing-without-airborne, landing advances early, fallback unchanged when never landed, out-of-radius landing doesn't count, backward compatibility with the pre-existing two-stage tests' bare fake envs, one-shot bonus fires once, bonus resets on new episode). **Known interaction, deliberately left as-is (2026-07-04, final-review finding):** the tier-RSI branch (`_tier_npz_reset`) seeds double/triple-step poses on the same wide crossings (`\|cross_y\| >= 0.6`) this gate applies to, and some of those NPZ frames start with the assigned foot already lifted — meaning `_blue_was_airborne` can be satisfied by the RSI reset pose itself rather than by the policy, and a foot that happens to land within 0.3 m of the blue target shortly after reset can latch `_blue_landed`/pay `blue_ball_landed` "for free" on tier-RSI episodes. User-directed decision: document only, no code change — the +10 bonus is small and one-shot next to `stopball` (100–250), episodes can't stall either way, and the 20% non-tier-RSI resets still require a genuine landing. When reviewing the next training run's double-stepping metrics, discount episodes that started from a tier-RSI wide-crossing reset accordingly. Not yet validated against a training run. **2026-07-04 hardening:** research (`Episode_Reward/blue_ball_landed` converted via this file's own documented formula — see "Reading TensorBoard" below — gives ~35-40% of all episodes, ~73-83% of wide crossings) showed the soft gate looked healthy on paper, but a Monte Carlo of `_tier_npz_reset`'s sampling plus user observation in `sgk_play` (0% visible genuine landings) showed this was almost entirely RSI free credit: tier-RSI resets draw a random frame from the DoubleStep/TripleStep motion clips, which can already have the assigned foot lifted near the blue target. Removing the fallback outright, rebalancing RSI (see the row below), and adding the genuine/RSI-assisted split were shipped together in one run — user's explicit choice over testing the RSI rebalance in isolation first — with the new diagnostic metrics as the agreed mitigation for that risk. Full rationale: `docs/superpowers/specs/2026-07-04-blue-ball-hard-gate-rsi-rebalance-design.md`. Covered by `tests/simple_goalkeeper/test_blue_ball_landing_gate.py` (13 tests) and the rewritten tests in `tests/simple_goalkeeper/test_reach_target_two_stage.py`. Not yet validated against a training run. |
| `stopball`/`softstop` gated on the blue-ball landing latch | N/A — G1 has no staged/waypoint target mechanism at all (see the parent two-stage-target row above); a landing-gated version of a mechanism that doesn't exist upstream has no G1 equivalent. | **Landing gate extended to the save rewards themselves (2026-07-05):** `stopball` and `softstop` now AND `landing_ok = ~env._blue_wide \| env._blue_landed` into their `fired` condition (each calls `_get_reach_target_y` first to keep the state fresh). Since `env._sb_flag`/`env._softstop_flag` are set from `fired`, this gates the underlying flags, not just each function's own returned reward — `_ball_is_behind`, `single_foot_save`, `cleanstop`, `inner_face_orientation_save`, and `foot_inner_face_continuous` (all keyed off `_softstop_flag`/`_softstop_correct_foot`) now also wait for a genuine landing on wide crossings. `_ball_is_behind`'s independent `ball_x_local < 0` condition still fires once the ball crosses the goal line regardless, so nothing can stall forever. Narrow crossings (`~env._blue_wide`) are unaffected. | **Deliberate divergence, closes the actual root cause of the "one big reach" behavior:** the 2026-07-04/2026-07-05 fixes above closed every path by which the two-stage *target* could be bypassed, but `stopball`/`softstop` themselves never referenced `env._blue_wide`/`env._blue_landed` at all — they fire purely off ball physics. Confirmed by user observation in `sgk_play` (checkpoint `model_4750`, run `2026-07-04_20-31-10`, no `--rsi` so no RSI-seeded free credit): robot made one continuous reach straight to the true crossing point with no visible two-stage stepping. That run's own `Episode_Metrics/blue_landed_genuine` (~23-26% of ALL episodes) is a clear minority even before restricting to wide-only, while `softstop`/`stopball` (~262.5/~37.5 max weight) are roughly an order of magnitude larger than `blue_ball_landed` (max 25, one-shot) — so skipping the waypoint and beelining for the true crossing point was always the higher-expected-reward strategy. **Risk, accepted not resolved:** hard-gating the two largest rewards in the table behind a genuine-landing rate this low risks sparsifying training signal sharply on wide crossings specifically — the same category of risk the 2026-07-04 hard gate itself needed an iteration cycle over. User's explicit choice to ship now and watch `stopball`/`softstop`/`blue_landed_genuine` rates and divergence on the next run rather than soften preemptively. Covered by `tests/simple_goalkeeper/test_stopball_softstop_landing_gate.py` (6 tests: wide+never-landed blocks both rewards, wide+landed fires both, narrow unaffected for both). 84/84 tests pass. Not yet validated against a training run. |
...
| Term | Weight | Purpose |
|------|--------|---------|
| `stopball` | +100→250 (curriculum) | One-time bonus when ball is deflected (delta_vx > 1 m/s). Primary signal. **2026-07-05: on wide crossings, also requires `env._blue_landed` (see divergence table) — cannot fire until the assigned foot has genuinely landed at the blue midpoint.** |
| `softstop` (`_softstop_flag`) | +105→262.5 (curriculum) | One-time bonus when the ball genuinely reverses (world-X velocity > threshold). Gates `_ball_is_behind` and every same-foot bonus (`single_foot_save`, `cleanstop`, `inner_face_orientation_save`, `foot_inner_face_continuous`). **2026-07-05: same wide-crossing landing gate as `stopball` — see divergence table.** |
