...
def dof_vel_limits(
    ...
    """Penalise joint velocities exceeding the soft velocity limit.

    Mirrors ILB dof_vel_limits exactly: linear penalty above vel_limit * soft_factor.
    T1 hardware limits are ~20 rad/s for legs; 10 rad/s was too conservative and
    fired on every fall, producing -30 per crashed episode and dominating training.
    G1 uses per-joint URDF velocity × 0.9 (linear); this uses 20*0.9=18 rad/s universal.
    """
    robot: Entity = env.scene[asset_cfg.name]
    vel = robot.data.joint_vel[:, asset_cfg.joint_ids]                  # (N, J)
    out_of_limit = (vel.abs() - vel_limit * soft_factor).clamp(min=0.0) # (N, J)
