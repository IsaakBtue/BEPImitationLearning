...
def motion_type_id(env: "ManagerBasedRlEnv") -> torch.Tensor:
    """Left/right discriminator zone encoded as 0.0 (left) or 1.0 (right). Shape (N, 1).

    Mirrors G1's end_regions / 3 observation term: gives the policy explicit knowledge
    of which AMP discriminator zone it belongs to so it can condition stepping direction.
    Set once at startup by assign_motion_types(); never changes during an episode.
    """
    ids = getattr(env, "_motion_type_ids", None)
    if ids is None:
        return torch.zeros(env.num_envs, 1, device=env.device)
    return ids.float().unsqueeze(-1)


def ball_landing_y_b(
    env: "ManagerBasedRlEnv",
    ball_name: str = "ball",
) -> torch.Tensor:
    """Estimated ball Y-position in robot body frame when it crosses the goal line (x_b=0).

    Mirrors G1's end_target_local observation: tells the policy WHERE the ball will land
    so it can plan foot placement before the ball arrives.  Computed by extrapolating
    the current ball trajectory linearly (valid for rolling-ball training).

    Returns shape (N, 1), clipped to ±2 m.  When the ball is behind the robot or
    stationary, returns ball's current Y position as a fallback.
    """
    robot: Entity = env.scene["robot"]
    ball: Entity = env.scene[ball_name]
    ...
    x_b  = ball_pos_b_val[:, 0]   # forward (positive = ball in front)
    y_b  = ball_pos_b_val[:, 1]   # lateral
    vx_b = ball_vel_b_val[:, 0]   # forward velocity (negative when approaching)
    vy_b = ball_vel_b_val[:, 1]   # lateral velocity

    # t_to_goal = -x_b / vx_b  (positive when ball is in front and approaching)
    # Clamp vx_b to avoid division by zero; also avoid backward-moving predictions.
    approaching = (x_b > 0.0) & (vx_b < -0.1)
    t_to_goal = torch.where(approaching, -x_b / vx_b.clamp(max=-0.1),
                            torch.zeros_like(x_b))
    y_land = y_b + vy_b * t_to_goal

    # Fall back to current y_b when ball is not approaching
    y_land = torch.where(approaching, y_land, y_b)

    return y_land.clamp(-2.0, 2.0).unsqueeze(-1)
