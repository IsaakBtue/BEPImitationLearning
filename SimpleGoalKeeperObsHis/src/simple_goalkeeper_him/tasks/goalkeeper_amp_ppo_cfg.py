...
def goalkeeper_amp_ppo_runner_cfg() -> RslRlAmpRunnerCfg:
    ...
    return RslRlAmpRunnerCfg(
        ...
        algorithm=RslRlPpoAlgorithmCfg(
            ...
            clip_param=0.2,
            entropy_coef=0.01,
            num_learning_epochs=5,  # matches G1 (LeggedRobotCfgPPO.algorithm) and ILB
            num_mini_batches=4,     # matches G1 and ILB — works once actuator gains are correct
            learning_rate=1.0e-3,
            schedule="adaptive",
            gamma=0.99,             # G1 (LeggedRobotCfgPPO.algorithm): 0.99 — overrides him_ppo.py default of 0.998
            lam=0.95,
            desired_kl=0.01,
            max_grad_norm=1.0,
        ),
        experiment_name="simple_goalkeeper_him",
        save_interval=200,
        num_steps_per_env=50,   # 50 vs G1's 100: halves rollout buffers (~3 GB) to fit 8192 envs on 32 GB GPU
        max_iterations=40_000,
        amp_coef=0.4,  # G1 value — kept at 0.4 now that task reward weights match G1 (eereach=10, stopball=100, hand_proximity=5)
