# Ported from Humanoid-Goalkeeper/rsl_rl/rsl_rl/storage/him_rollout_storage.py
# Removed unused split_and_pad_trajectories import.
...
class HIMRolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape,
                 actions_shape, amp_obs_shape, device='cpu'):
        self.device = device
        ...
        if privileged_obs_shape[0] is not None:
            ...
            self.next_privileged_observations = torch.zeros(num_transitions_per_env, num_envs,
                                                            *privileged_obs_shape, device=self.device)
        else:
        ...
        self.amp_observations = torch.zeros(num_transitions_per_env, num_envs, *amp_obs_shape,
                                            device=self.device)
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield (obs_batch, next_obs_batch, critic_observations_batch, actions_batch,
                       next_critic_observations_batch, cont_batch, target_values_batch,
                       advantages_batch, returns_batch, old_actions_log_prob_batch,
                       old_mu_batch, old_sigma_batch, amp_obs_batch)
