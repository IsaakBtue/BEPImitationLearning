...
def reset_from_motion_data(
    ...
    FIX 2026-09-13 (user request, "revert that pull and make sure that rsi
    is turned off so 100% standing pose just like husky" -- a deliberate
    divergence from G1's own 80/20, NOT a parity fix): 0.8 -> 0.0. Every
    reset now unconditionally takes the default-pose + randomization
    branch (`MotionResetManager.reset`'s `else` branch) -- no donor-copy
    RSI at all. Matches HUSKY's own actual approach (arXiv 2602.03205:
    "all parallel envs initialize from Pushing," their fixed start phase,
    every episode -- no reference-state diversity) rather than DeepMimic's
    RSI-ablation evidence (Peng et al. 2018, Table 5: RSI matters a lot for
    hard dynamic skills). This is the opposite direction from that
    evidence, a deliberate one-run experiment per explicit user request,
    not a claim that fixed-start is better for this task.
    """
    mgr = MotionResetManager.get()
    mgr.init(env)
    mgr.reset(env, env_ids, asset_cfg, rsi_fraction=0.0)
...
def _update_smoothed_softstop_success(
    ...
    task success (env._softstop_flag -- a real save happened, not a reward
    magnitude) as their progress signal, matching OpenAI's ADR (arXiv
    1910.07113), which explicitly uses success rate rather than reward for
    exactly this kind of threshold. Deliberately NOT reading
    Episode_Reward/softstop (the weighted reward value) -- that number is
    entangled with softstop's OWN curriculum-ramped weight
    (softstop_curriculum), so the same underlying success rate would read
    differently depending on where that unrelated curriculum happens to be.
    _softstop_flag itself is weight-independent: reset False at episode
...
class domain_rand_curriculum:
    ...
    practice at low visibility. Registered as its own curriculum term (not
