...
**Third health check** (`model_2500.pt`, ~900 post-saturation iterations): live diagnostic measured **90.8% genuine (1227/1351 wide episodes), 0% RSI-assisted** — within noise of the 96.6% reading, still holding, no sign of the historical decay in the diagnostic's numbers.

**Stopped 2026-07-14 by explicit user direction** ("it didn't learn it"), based on the user's own direct observation of the checkpoint's behavior rather than the landing-rate diagnostic alone — the diagnostic's high genuine-landing numbers evidently don't capture the full picture of whether the task was actually learned well. `model_2500.pt` was already pushed to both `blue-ball-waypoint` and `master` (commits `f9bb2f4`/`e156268`) before this stop. Process (PID `1300789` + wandb children `1300901`/`1300915`) killed cleanly. **This is a reminder that the live landing-rate diagnostic, while a trustworthy check against the specific "genuine settle-window landing" event, is not a substitute for actually watching the policy play** — it can look statistically great on the one metric it measures while still failing the user's broader judgment of the task. No new blue training running as of this entry.
